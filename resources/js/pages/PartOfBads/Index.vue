...
import { Boxes, CheckCircle2, ChevronLeft, ChevronRight, PencilLine, Plus, Search, Trash2, XCircle } from 'lucide-vue-next';
import { computed, ref, watch } from 'vue';

interface ManagedPartOfBad {
    id: number;
    name: string;
    is_active: boolean;
...
const props = defineProps<{
    partOfBads: Paginator<ManagedPartOfBad>;
...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'Part Of Bad', href: '/part-of-bads' },
];
...
const activePartOfBad = ref<ManagedPartOfBad | null>(null);

const canCreatePartOfBads = computed(() => page.props.auth.can.create_part_of_bads);
const canUpdatePartOfBads = computed(() => page.props.auth.can.update_part_of_bads);
const canDeletePartOfBads = computed(() => page.props.auth.can.delete_part_of_bads);

const createForm = useForm({ name: '', is_active: true });
const editForm = useForm({ name: '', is_active: true });
const deleteForm = useForm({});

const pageData = computed(() => props.partOfBads);
const rows = computed(() => props.partOfBads.data ?? []);
const paginationLinks = computed(() => props.partOfBads.links?.filter((link) => link.url) ?? []);
...
const openEditModal = (partOfBad: ManagedPartOfBad) => {
    activePartOfBad.value = partOfBad;
    editForm.defaults({ name: partOfBad.name, is_active: partOfBad.is_active });
    editForm.reset();
    editForm.clearErrors();
    isEditOpen.value = true;
};

const openDeleteModal = (partOfBad: ManagedPartOfBad) => {
    activePartOfBad.value = partOfBad;
...
const closeDeleteModal = () => {
    ...
    activePartOfBad.value = null;
    deleteForm.clearErrors();
};

const submitCreate = () =>
    createForm.post(route('part-of-bads.store'), {
        preserveScroll: true,
        onSuccess: () => {
...
const submitEdit = () => {
    if (!activePartOfBad.value) return;
    editForm.put(route('part-of-bads.update', activePartOfBad.value.id), {
        preserveScroll: true,
        onSuccess: () => {
...
const submitDelete = () => {
    if (!activePartOfBad.value) return;
    deleteForm.delete(route('part-of-bads.destroy', activePartOfBad.value.id), {
        preserveScroll: true,
            ...
            <div class="mx-auto max-w-7xl space-y-4">
                <section class="app-table-shell overflow-hidden">
                    <div class="grid gap-4 border-b border-[var(--app-border)] bg-[linear-gradient(135deg,_#eef4ff_0%,_#f8fbff_100%)] px-5 py-4 text-[var(--app-ink)] lg:grid-cols-[1.2fr,0.8fr]">
                        <div>
                            ...
                            <h1 class="mt-1 text-xl font-black tracking-tight">Part Of Bad Directory</h1>
                            <p class="mt-1 text-xs font-medium text-slate-500">Kelola daftar area kerusakan agar pelaporan complaint lebih konsisten.</p>
                        </div>
                        <div v-if="canCreatePartOfBads" class="flex items-center justify-end">
                ...
                <section class="app-table-shell p-5">
                    <div class="grid gap-4 xl:grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)] xl:items-center">
                        <div>
                            <h2 class="text-base font-black text-slate-900 uppercase tracking-wide">Part Of Bad List</h2>
                        </div>
                        <div class="grid gap-2 sm:grid-cols-2 lg:flex lg:justify-end">
                            <div class="relative lg:w-64">
                                ...
                                <Input v-model="search" class="h-9 pl-9 text-xs" placeholder="Cari part..." />
                            </div>
                            ...
                            <table class="min-w-full divide-y divide-slate-100 text-[13px]">
                                <thead class="bg-slate-50/50">
                                    <tr class="text-left text-[10px] font-black uppercase tracking-widest text-slate-400">
                                        <th class="px-4 py-3">Part Of Bad</th>
                                        <th class="px-4 py-3">Status</th>
                                        <th class="px-4 py-3 text-right">Action</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-slate-50 bg-white">
                                    <tr v-for="partOfBad in rows" :key="partOfBad.id" class="transition hover:bg-slate-50/50">
                                        <td class="px-4 py-3">
                                            <div class="flex items-center gap-2.5">
                                                ...
                                                <div>
                                                    <p class="font-bold text-slate-900 leading-tight">{{ partOfBad.name }}</p>
                                                    <p class="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">ID: #{{ partOfBad.id }}</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="px-4 py-3">
                                            <span class="inline-flex rounded-full px-2 py-0.5 text-[9px] font-black uppercase tracking-wider shadow-sm" :class="statusBadgeClass(partOfBad.is_active)">{{ partOfBad.is_active ? 'Active' : 'Inactive' }}</span>
                                        </td>
                                        <td class="px-4 py-3 text-right">
                                            <div class="flex justify-end gap-1.5">
                                                <Button v-if="canUpdatePartOfBads" type="button" variant="ghost" size="sm" class="h-8 rounded-lg text-slate-400 hover:text-[var(--app-primary)] hover:bg-blue-50" @click="openEditModal(partOfBad)">
                                                    <PencilLine class="h-3.5 w-3.5" />
                                                </Button>
                                                <Button v-if="canDeletePartOfBads" type="button" variant="ghost" size="sm" class="h-8 rounded-lg text-slate-400 hover:text-rose-600 hover:bg-rose-50" @click="openDeleteModal(partOfBad)">
        ...
        <Dialog v-model:open="isCreateOpen">
            <DialogContent class="max-w-xl overflow-hidden rounded-[28px] border-0 p-0 shadow-[0_30px_80px_rgba(15,23,42,0.25)]">
                <div class="bg-[#EEF2FF] px-7 py-8">
                    <DialogHeader>
                        <DialogTitle class="text-3xl font-black text-slate-900">Tambah Part Of Bad</DialogTitle>
                        <DialogDescription class="mt-2 text-base font-medium text-slate-500">Masukkan pilihan area kerusakan baru agar siap dipakai di flow complaint.</DialogDescription>
                    </DialogHeader>
                </div>
                ...
                <form class="space-y-6 bg-white px-7 py-7" @submit.prevent="submitCreate">
                    <div class="space-y-4">
                        <div class="grid gap-2">
                            <Label for="create-name" class="text-[13px] font-bold uppercase tracking-wide text-slate-700">Nama Part</Label>
                            <Input id="create-name" v-model="createForm.name" placeholder="Contoh: Packaging" class="h-12 rounded-xl border-slate-200 bg-slate-50/50 px-4 text-sm transition focus:bg-white" />
                        ...
                        <div class="flex items-center gap-3 rounded-2xl border border-slate-100 bg-slate-50/50 px-4 py-4 transition hover:bg-slate-50">
                            ...
                            <span class="text-sm font-semibold text-slate-600">Part aktif dan siap dipakai di modul complaint</span>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center justify-end gap-3 border-t border-slate-100 pt-6">
                        ...
                        <Button :disabled="createForm.processing" class="h-11 rounded-xl bg-[var(--app-primary)] px-6 font-bold text-white shadow-lg shadow-blue-500/20 hover:bg-[var(--app-primary-dark)]">
                            ...
                            {{ createForm.processing ? 'Creating...' : 'Create Part' }}
                        </Button>
                    </div>
                </form>
            </DialogContent>
        ...
        <Dialog v-model:open="isEditOpen">
            <DialogContent class="max-w-xl overflow-hidden rounded-[28px] border-0 p-0 shadow-[0_30px_80px_rgba(15,23,42,0.25)]">
                <div class="bg-slate-900 px-7 py-8">
                    <DialogHeader>
                        <DialogTitle class="text-3xl font-black text-white">Edit Part Of Bad</DialogTitle>
                        <DialogDescription class="mt-2 text-base font-medium text-slate-400">Perbarui pilihan area kerusakan agar operasional tetap konsisten.</DialogDescription>
                    </DialogHeader>
                </div>

                <form v-if="activePartOfBad" class="space-y-6 bg-white px-7 py-7" @submit.prevent="submitEdit">
                    <div class="space-y-4">
                        <div class="grid gap-2">
                            <Label for="edit-name" class="text-[13px] font-bold uppercase tracking-wide text-slate-700">Nama Part</Label>
                        ...
                        <div class="flex items-center gap-3 rounded-2xl border border-slate-100 bg-slate-50/50 px-4 py-4 transition hover:bg-slate-50">
                            ...
                            <span class="text-sm font-semibold text-slate-600">Part aktif</span>
                        </div>
                    </div>
        ...
        <Dialog v-model:open="isDeleteOpen">
            <DialogContent class="max-w-md overflow-hidden rounded-[28px] border-0 p-0 shadow-[0_30px_80px_rgba(244,63,94,0.15)]">
                <div class="bg-rose-50 px-7 py-8">
                    <DialogHeader>
                        <DialogTitle class="text-3xl font-black text-rose-950">Hapus Part</DialogTitle>
                ...
                <div class="space-y-6 bg-white px-7 py-7">
                    <div v-if="activePartOfBad" class="rounded-2xl border border-rose-100 bg-rose-50/30 p-5">
                        <p class="text-sm font-bold text-rose-900 uppercase tracking-tight">Konfirmasi Part</p>
                        <p class="mt-1 text-lg font-black text-rose-600">{{ activePartOfBad.name }}</p>
                    </div>
                    ...
                    <div class="flex items-center justify-end gap-3 border-t border-slate-100 pt-6">
                        ...
                        <Button type="button" class="h-11 rounded-xl bg-rose-600 px-6 font-bold text-white shadow-lg shadow-rose-500/20 hover:bg-rose-700" :disabled="deleteForm.processing" @click="submitDelete">
                            ...
                            {{ deleteForm.processing ? 'Deleting...' : 'Delete Part' }}
                        </Button>
                    </div>
                </div>
            </DialogContent>
