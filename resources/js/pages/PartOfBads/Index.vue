                ...
                <section class="app-table-shell overflow-hidden">
                    <div class="grid gap-6 border-b border-[var(--app-border)] bg-[linear-gradient(135deg,_#eef4ff_0%,_#f8fbff_100%)] px-6 py-7 text-[var(--app-ink)] lg:grid-cols-[1.2fr,0.8fr]">
                        <div>
                            ...
                            <h1 class="mt-3 text-3xl font-extrabold tracking-tight">Part Of Bad Directory</h1>
                            <p class="mt-3 max-w-2xl text-sm leading-6 text-slate-500">Kelola daftar part yang bermasalah agar field Part of Bad di complaint konsisten dan mudah dipilih.</p>
                        </div>
                        <div v-if="canCreatePartOfBads" class="flex items-start justify-end">
                ...
                <section class="app-table-shell p-6">
                    <div class="grid gap-5 xl:grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)] xl:items-end">
                        <div class="min-w-0 max-w-xl">
                            <h2 class="text-xl font-semibold text-slate-900">Part Of Bad List</h2>
                            <p class="mt-1 max-w-md text-sm leading-6 text-slate-500">Master ini membantu tim memilih area kerusakan tanpa input bebas yang berbeda-beda.</p>
                        </div>
                        <div class="grid gap-3 sm:grid-cols-2 xl:grid-cols-[minmax(0,1.5fr)_minmax(0,0.8fr)]">
                            <div class="relative">
                                <Search class="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
                                <Input v-model="search" class="pl-10" placeholder="Cari part of bad..." />
                            </div>
                            ...
                            <table class="min-w-full divide-y divide-slate-200 text-sm">
                                <thead class="bg-slate-50/90">
                                    <tr class="text-left text-slate-500">
                                        <th class="px-5 py-4 font-medium">Part Of Bad</th>
                                ...
                                <tbody class="divide-y divide-slate-100 bg-white">
                                    <tr v-for="partOfBad in rows" :key="partOfBad.id" class="transition hover:bg-slate-50/70">
                                        <td class="px-5 py-4">
                                            <div class="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <p class="font-medium text-slate-900">{{ partOfBad.name }}</p>
                                                    <p class="mt-1 text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">Master Part Of Bad</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="px-5 py-4">
                                            <span class="inline-flex rounded-full px-3 py-1 text-xs font-semibold" :class="statusBadgeClass(partOfBad.is_active)">{{ partOfBad.is_active ? 'Active' : 'Inactive' }}</span>
                                        </td>
                                        <td class="px-5 py-4 text-slate-500">{{ formatDate(partOfBad.created_at) }}</td>
                                        <td class="px-5 py-4">
                                            <div class="flex justify-end gap-2">
                                                <Button v-if="canUpdatePartOfBads" type="button" variant="outline" size="sm" class="rounded-xl" @click="openEditModal(partOfBad)">
                                                    <PencilLine class="h-4 w-4" />
                                                    Edit
                                                </Button>
                                                <Button v-if="canDeletePartOfBads" type="button" variant="destructive" size="sm" class="rounded-xl" @click="openDeleteModal(partOfBad)">
                                    ...
                                    <tr v-if="rows.length === 0">
                                        <td colspan="4" class="px-5 py-14 text-center">
                                            <div class="mx-auto max-w-sm space-y-2">
                                                ...
                                                <p class="font-medium text-slate-900">Tidak ada part of bad yang cocok</p>
                    ...
                    <div class="mt-5 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                        <p class="text-sm text-slate-500">Menampilkan {{ pageData.from || 0 }} - {{ pageData.to || 0 }} dari {{ pageData.total }} part</p>
            ...
            <DialogContent class="max-w-xl rounded-[28px] border-0 p-0 shadow-[0_30px_80px_rgba(15,23,42,0.25)]">
                <div class="overflow-hidden rounded-[28px]">
                    <div class="bg-[linear-gradient(135deg,_#eef4ff_0%,_#dbeafe_100%)] px-6 py-6 text-[var(--app-ink)]">
                        <DialogHeader>
                            <DialogTitle class="text-2xl">Tambah Part Of Bad</DialogTitle>
                            <DialogDescription class="text-slate-500">Masukkan pilihan part baru agar siap dipakai di flow complaint.</DialogDescription>
                        </DialogHeader>
                    </div>
                    <form class="space-y-5 bg-white px-6 py-6" @submit.prevent="submitCreate">
                        <div class="grid gap-2">
                            <Label for="create-name">Nama Part</Label>
                            <Input id="create-name" v-model="createForm.name" placeholder="Contoh: Packaging" />
                        ...
                        <label class="flex items-center gap-3 text-sm text-slate-600">
                            ...
                            Part aktif dan siap dipakai di complaint
                        </label>
            ...
            <DialogContent class="max-w-xl rounded-[28px] border-0 p-0 shadow-[0_30px_80px_rgba(15,23,42,0.25)]">
                <div class="overflow-hidden rounded-[28px]">
                    <div class="bg-[linear-gradient(135deg,_#0f172a_0%,_#1e293b_100%)] px-6 py-6 text-white">
                        <DialogHeader>
                            <DialogTitle class="text-2xl">Edit Part Of Bad</DialogTitle>
                            <DialogDescription class="text-slate-200">Perbarui pilihan part yang dipakai flow complaint.</DialogDescription>
                        </DialogHeader>
                    </div>
                    <form v-if="activePartOfBad" class="space-y-5 bg-white px-6 py-6" @submit.prevent="submitEdit">
                        <div class="grid gap-2">
                            <Label for="edit-name">Nama Part</Label>
            ...
            <DialogContent class="max-w-lg rounded-[28px] border-0 p-0 shadow-[0_30px_80px_rgba(15,23,42,0.25)]">
                <div class="overflow-hidden rounded-[28px]">
                    <div class="bg-rose-50 px-6 py-6 text-rose-700">
                        <DialogHeader>
                            <DialogTitle class="text-2xl text-rose-700">Hapus Part Of Bad</DialogTitle>
                            <DialogDescription class="text-rose-600">Pastikan part ini memang tidak lagi dipakai di flow complaint.</DialogDescription>
                        </DialogHeader>
                    </div>
                    <div v-if="activePartOfBad" class="space-y-5 bg-white px-6 py-6">
                        <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                            <p class="font-medium text-slate-900">{{ activePartOfBad.name }}</p>
                            <span class="mt-2 inline-flex rounded-full px-3 py-1 text-xs font-semibold" :class="statusBadgeClass(activePartOfBad.is_active)">{{ activePartOfBad.is_active ? 'Active' : 'Inactive' }}</span>
                        </div>
                        <DialogFooter>
