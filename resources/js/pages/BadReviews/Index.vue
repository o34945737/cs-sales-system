...
const currentStar = computed(() => filterState.value.priority || 'All');
...
const filteredCsSummary = computed(() => {
    if (!agentSearchQuery.value) return csSummary.value;
    const query = agentSearchQuery.value.toLowerCase();
    return csSummary.value.filter((cs) =>
        cs.cs_name.toLowerCase().includes(query)
    );
});
...
const starCards = computed(() => [
    { key: 'All', label: 'All Star', value: starSummary.value.all || 0 },
    { key: '1', label: '★ 1', value: starSummary.value['1'] || 0 },
    { key: '2', label: '★ 2', value: starSummary.value['2'] || 0 },
    { key: '3', label: '★ 3', value: starSummary.value['3'] || 0 },
]);
...
const overviewCards = computed(() => [
    ...
    { label: 'Assigned CS', value: csSummary.value.length || 0 },
]);
...
const readonlyInputClass = 'h-11 w-full rounded-lg border border-slate-200/60 bg-slate-50 px-4 py-2.5 text-[13px] font-bold text-slate-600 outline-none cursor-not-allowed';
const controlClass = (field, type = 'input') => {
    const baseClass = 'h-11 w-full rounded-lg border px-4 py-2.5 text-[13px] font-bold outline-none transition-all';
    const hasError = form.errors[field];
    const errorClass = hasError ? 'border-rose-300 bg-rose-50/50 focus:ring-4 focus:ring-rose-500/10' : 'border-slate-200/60 bg-white focus:ring-4 focus:ring-blue-500/10 hover:border-slate-300';

    if (type === 'select') {
        return `${baseClass} appearance-none pr-10 ${errorClass}`;
    }
...
const setStar = (star) => visitIndex({ priority: star === 'All' ? undefined : star, page: 1 });
...
const visitIndex = (params = {}, options = {}) => {
    router.get(
...
watch(
    search,
    debounce((value) => {
        visitIndex({ search: value || undefined, page: 1 });
    }, 300)
);
...
const submitForm = async () => {
    if (modalMode.value === 'create') {
        form.post(route('bad-reviews.store'), {
            onSuccess: () => {
                discardForm();
            },
        });
    } else {
        form.put(route('bad-reviews.update', detailItem.value?.id), {
            onSuccess: () => {
                discardForm();
            },
        });
...
const deleteRecord = () => {
    form.delete(route('bad-reviews.destroy', deleteItem.value.id), {
        onSuccess: () => {
                    ...
                    <div class="rounded-3xl border border-slate-100 bg-slate-50/50 p-2 shadow-sm ring-1 ring-slate-100/10">
                        <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                            ...
                            <div class="flex flex-wrap items-center gap-2 pr-1">
                                ...
                                <!-- Status Select -->
                                <div class="relative min-w-[130px]">
                                    <select :value="currentStatus" class="h-10 w-full appearance-none rounded-xl border border-slate-200/60 bg-white pl-4 pr-10 text-[11px] font-black uppercase tracking-wider text-slate-600 outline-none transition-all hover:border-slate-300 focus:ring-4 focus:ring-blue-50/50 shadow-sm" @change="setStatus($event.target.value)">
                            ...
                            <div class="rounded-[24px] border border-slate-100 bg-white p-5 shadow-sm ring-1 ring-slate-100/50">
                                <header class="flex items-center justify-between">
                                    <div>
                                        ...
                                        <h2 class="mt-0.5 text-lg font-black text-slate-900">Review Assignees</h2>
                                    </div>
                                ...
                                <div class="mt-6 space-y-2.5">
                                    ...
                                    <div class="relative group">
                                        ...
                                        <input v-model="agentSearchQuery" type="text" placeholder="Search reviewer..." class="h-9 w-full rounded-xl border border-slate-100 bg-slate-50/50 pl-9 pr-4 text-[11px] font-bold text-slate-700 outline-none transition-all focus:border-[var(--app-primary)] focus:bg-white focus:ring-4 focus:ring-blue-50/50 placeholder:font-medium placeholder:text-slate-400" />
                                    </div>

                                    <div class="space-y-1.5 mt-4 max-h-[360px] overflow-y-auto pr-1.5 border-b border-dashed border-slate-100 pb-3">
                                        <p class="text-[9px] font-black uppercase tracking-[0.15em] text-slate-400 mb-2">Select Reviewer</p>
                                        <button v-for="cs in filteredCsSummary" :key="cs.cs_name" @click="setCsFilter(cs.cs_name)" class="w-full group p-3 flex items-center justify-between rounded-xl border transition-all text-left" :class="currentCs === cs.cs_name ? 'border-[var(--app-primary)] bg-blue-50/50 ring-1 ring-[var(--app-primary)]/10' : 'border-slate-50 bg-white hover:border-slate-200'">
                                            <div>
                                                <p class="text-[13px] font-black text-slate-900 leading-tight">{{ cs.cs_name }}</p>
                                                <p class="text-[10px] font-bold text-slate-400 mt-0.5">{{ cs.total }} reviews</p>
                                            </div>
                                        ...
                                        <div v-if="filteredCsSummary.length === 0" class="py-10 text-center">
                                            ...
                                            <p class="mt-2 text-[11px] font-bold text-slate-400 uppercase tracking-widest">No reviewer found</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <section class="app-table-shell p-0">
                                <div class="flex flex-col gap-6 border-b border-slate-100 px-6 py-7 lg:flex-row lg:items-center lg:justify-between">
                                    <div class="min-w-0">
                                        ...
                                        <h2 class="mt-1 text-2xl font-black text-slate-900 tracking-tight">Review Management</h2>
                                        <div class="mt-2.5 flex items-center gap-2">
                                            ...
                                            <div class="rounded-full border px-3 py-1 text-[10px] font-black uppercase tracking-widest" :class="activeFilterCount ? 'border-amber-200 bg-amber-50 text-amber-600 shadow-sm shadow-amber-500/5' : 'border-slate-100 bg-white text-slate-400'">
                                    ...
                                    <div class="flex flex-1 flex-col gap-4 lg:max-w-xl lg:flex-row lg:items-center lg:justify-end">
                                        <div class="relative flex-1 group">
                                            ...
                                            <input v-model="search" type="text" placeholder="Quick search reviews..." class="h-12 w-full rounded-2xl border-slate-200/60 bg-slate-50/50 pl-11 pr-4 text-[13px] font-bold text-slate-700 placeholder:font-medium placeholder:text-slate-400 outline-none transition-all focus:border-[var(--app-primary)] focus:bg-white focus:ring-[6px] focus:ring-blue-500/5 shadow-inner" />
                                        </div>

                                        <button type="button" class="group flex h-12 items-center justify-center gap-2 rounded-2xl bg-[var(--app-primary)] px-6 text-[14px] font-black text-white shadow-[0_15px_30px_rgba(53,103,232,0.25)] transition-all hover:bg-[var(--app-primary-dark)] hover:-translate-y-1 hover:shadow-[0_20px_40px_rgba(53,103,232,0.35)] active:scale-[0.98]" @click="openCreateModal">
                                            <Plus class="h-5 w-5 stroke-[4px]" />
                                            <span class="hidden xl:inline">Create Review</span>
                                        </button>
                                    </div>
                                </div>
                                    ...
                                    <table class="w-full min-w-[1080px] table-fixed divide-y divide-[var(--line)]">
                                        <thead class="bg-slate-50/80">
                                            <tr class="text-left text-[10px] font-black uppercase tracking-widest text-slate-400">
                                                <th class="w-[12%] py-3 pl-5 pr-4">Order ID</th>
                                                <th class="w-[15%] px-4 py-3">Customer</th>
                                                <th class="w-[10%] px-4 py-3">Brand</th>
                                                <th class="w-[10%] px-4 py-3 text-center">Star</th>
                                                <th class="w-[12%] px-4 py-3">Category</th>
                                                <th class="w-[10%] px-4 py-3">Progress</th>
                                                <th class="w-[10%] px-4 py-3 text-center">Status</th>
                                                <th class="w-[12%] px-4 py-3">Reviewer</th>
                                                <th class="w-[12%] py-3 pl-4 pr-5 text-right">Actions</th>
                                            </tr>
                                        </thead>
                                        <tbody class="divide-y divide-[var(--line)] bg-white">
                                            <tr v-for="item in badReviewRows" :key="item.id" class="group align-top transition-colors hover:bg-slate-50/70">
                                                <td class="py-3 pl-5 pr-4 text-[13px] font-bold text-[var(--app-ink)]">{{ item.order_id || '-' }}</td>
                                                <td class="px-4 py-3">
                                                    <div class="space-y-0.5">
                                                        <p class="truncate text-[13px] font-bold text-[var(--app-ink)]">{{ item.username || '-' }}</p>
                                                        <p class="truncate text-[10px] font-bold text-slate-400 uppercase tracking-tighter">{{ item.platform || '-' }}</p>
                                                    </div>
                                                </td>
                                                <td class="px-4 py-3 text-[13px] font-medium text-slate-600">{{ item.brand || '-' }}</td>
                                                <td class="px-4 py-3 text-center">
                                                    <span class="inline-flex items-center gap-1 text-[12px] font-black">
                                                        <Star class="h-3 w-3 fill-amber-400 text-amber-400" />{{ item.star || '-'
                                                        }}
                                                    </span>
                                                </td>
                                                <td class="px-4 py-3 text-[13px] font-medium text-slate-600">{{ item.category_review || '-' }}</td>
                                                <td class="px-4 py-3">
                                                    <span class="inline-flex rounded-full px-2 py-0.5 text-[9px] font-black uppercase tracking-wider" :class="item.progress === 'Auto Reply' ? 'bg-blue-50 text-blue-600' : 'bg-amber-50 text-amber-600'">{{ item.progress || '-' }}</span>
                                                </td>
                                                <td class="px-4 py-3 text-center">
                                                    <span class="inline-flex rounded-full px-2 py-0.5 text-[9px] font-black uppercase tracking-wider" :class="item.status === 'Solved' ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'">{{ item.status || 'Pending' }}</span>
                                                </td>
                                                <td class="px-4 py-3 text-[13px] font-bold text-slate-600">{{ item.cs_name || 'UNASSIGNED' }}</td>
                                                <td class="py-3 pl-4 pr-5">
                                                    <div class="flex items-center justify-end gap-1.5">
                                                        <button type="button" class="group inline-flex h-8 w-8 items-center justify-center rounded-lg border border-slate-100 bg-white text-slate-400 shadow-sm transition-all hover:border-blue-100 hover:bg-blue-50/50 hover:text-blue-600" title="Detail" @click="openDetail(item)">
                                                            <Eye class="h-3.5 w-3.5" />
                                                        </button>
                                                        <button type="button" class="group inline-flex h-8 w-8 items-center justify-center rounded-lg border border-slate-100 bg-white text-slate-400 shadow-sm transition-all hover:border-amber-100 hover:bg-amber-50/50 hover:text-amber-600" title="Edit" @click="openEditModal(item)">
                                                            <Pencil class="h-3.5 w-3.5" />
                                                        </button>
                                                        <button type="button" class="group inline-flex h-8 w-8 items-center justify-center rounded-lg border border-slate-100 bg-white text-slate-400 shadow-sm transition-all hover:border-red-100 hover:bg-red-50/50 hover:text-red-600" title="Delete" @click="confirmDelete(item)">
                                ...
                                <div v-if="!badReviewRows.length" class="border-t border-slate-50 px-6 py-24 text-center">
                                    <div class="mx-auto max-w-sm space-y-5">
                                        ...
                                        <div class="space-y-2">
                                            <h3 class="text-2xl font-black tracking-tight text-[var(--app-ink)]">No Reviews Found</h3>
                                        </div>
                                        <button type="button" class="group inline-flex items-center justify-center gap-3 rounded-2xl bg-[var(--app-primary)] px-8 py-4 text-sm font-black text-white shadow-[0_12px_30px_rgba(53,103,232,0.25)] transition-all hover:bg-[var(--app-primary-dark)] hover:-translate-y-1 active:scale-95" @click="openCreateModal">
                                ...
                                <div class="flex flex-col gap-5 border-t border-slate-100 bg-slate-50/30 px-6 py-6 sm:flex-row sm:items-center sm:justify-between">
                                    ...
                                    <div class="flex flex-wrap items-center gap-1.5">
                                        <template v-for="(link, index) in paginationLinks" :key="index">
                                            <button v-if="link.url || link.active" type="button" class="flex h-10 min-w-[40px] items-center justify-center rounded-xl px-3 text-[13px] font-black transition-all" :class="link.active ? 'bg-[var(--app-primary)] text-white shadow-lg shadow-blue-500/20' : link.url ? 'bg-white text-slate-600 hover:border-slate-300 hover:bg-slate-50 ring-1 ring-slate-200/60' : 'cursor-not-allowed bg-slate-50 text-slate-300'" :disabled="!link.url" @click="link.url && router.visit(link.url, { preserveScroll: true, preserveState: true, replace: true })">
        ...
        <div v-if="detailItem" class="fixed inset-0 z-40 bg-slate-950/20 backdrop-blur-[1px]" @click.self="closeDetail">
            <aside class="absolute right-0 top-0 h-full w-full max-w-xl overflow-y-auto border-l border-[var(--line)] bg-white shadow-2xl">
                <div class="sticky top-0 z-10 flex items-center justify-between border-b border-slate-100 bg-white/80 px-5 py-4 backdrop-blur-md">
                    <div>
                        <p class="text-[9px] font-black uppercase tracking-[0.2em] text-[var(--app-primary)]">Review Detail</p>
                        <h3 class="mt-0.5 text-xl font-black text-[var(--app-ink)]">{{ detailItem.order_id || 'Review Detail' }}</h3>
                    </div>
                    <button type="button" class="flex h-9 w-9 items-center justify-center rounded-full bg-slate-50 text-slate-400 transition hover:bg-rose-50 hover:text-rose-500" @click="closeDetail">
                        <X class="h-4 w-4" />
                    </button>
                </div>
                ...
                <div class="space-y-8 px-7 py-7">
                    <div class="grid gap-4 sm:grid-cols-2">
                        <div class="relative overflow-hidden rounded-3xl border border-slate-100 bg-slate-50/50 p-5">
                            ...
                            <div class="mt-3 inline-flex rounded-full px-3.5 py-1.5 text-[11px] font-black uppercase tracking-wider shadow-sm" :class="detailItem.status === 'Solved' ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'">
                                {{ detailItem.status || 'Pending' }}
                            </div>
                        </div>
                        <div class="relative overflow-hidden rounded-3xl border border-slate-100 bg-slate-50/50 p-5">
                            <p class="relative z-10 text-[10px] font-bold uppercase tracking-[0.2rem] text-slate-400">Star Rating</p>
                            <div class="mt-3 inline-flex items-center gap-1 rounded-full px-3.5 py-1.5 text-[11px] font-black uppercase tracking-wider shadow-sm bg-amber-50 text-amber-600">
                    ...
                    <div class="grid gap-4 sm:grid-cols-2">
                        ...
                        <div class="rounded-3xl border border-slate-100 p-5 shadow-sm">
                            <p class="text-[10px] font-bold uppercase tracking-[0.2rem] text-slate-400">Assigned Reviewer</p>
                            <p class="mt-3 text-lg font-black text-[var(--app-ink)]">{{ detailItem.cs_name || 'UNASSIGNED' }}</p>
                            <p class="mt-1 text-[13px] font-medium text-slate-400 italic">"{{ detailItem.category_review || '-' }}"</p>
                        </div>
                    </div>
                    ...
                    <div class="space-y-6 rounded-[2rem] border border-slate-100 bg-[#f8fbff]/50 p-7 ring-1 ring-slate-100/50">
                        <div class="grid gap-y-6 gap-x-4 sm:grid-cols-2">
                            <div>
                                <p class="text-[10px] font-bold uppercase tracking-[0.15rem] text-slate-400">Tanggal Review</p>
                                <p class="mt-2 text-[15px] font-bold text-[var(--app-ink)]">{{ detailItem.tanggal_review || '-' }}</p>
                            </div>
                            <div>
                                <p class="text-[10px] font-bold uppercase tracking-[0.15rem] text-slate-400">Tanggal Update</p>
                                <p class="mt-2 text-[15px] font-bold text-[var(--app-ink)]">{{ detailItem.tanggal_update || '-' }}</p>
                            </div>
                            <div>
                            ...
                            <div>
                                <p class="text-[10px] font-bold uppercase tracking-[0.15rem] text-slate-400">Product SKU</p>
                                <p class="mt-2 text-[15px] font-bold text-[var(--app-ink)]">{{ detailItem.sku || '-' }}</p>
                            </div>
                            <div class="sm:col-span-2">
                                <p class="text-[10px] font-bold uppercase tracking-[0.15rem] text-slate-400">Product Name</p>
                                <p class="mt-2 text-[15px] font-bold text-[var(--app-ink)]">{{ detailItem.product_name || '-' }}</p>
                            </div>
                        </div>

                        <div class="border-t border-slate-200/50 pt-5">
                            <p class="text-[10px] font-bold uppercase tracking-[0.15rem] text-slate-400">Review Notes</p>
                            <p class="mt-3 text-[15px] leading-relaxed font-medium text-slate-600">{{ detailItem.review_notes || '-' }}</p>
                        </div>

                        <div class="border-t border-slate-200/50 pt-5">
                            <p class="text-[10px] font-bold uppercase tracking-[0.15rem] text-slate-400">Progress Status</p>
                            <p class="mt-3 text-[15px] font-bold text-slate-700">{{ detailItem.progress || '-' }}</p>
                        </div>
                    </div>
                </div>
            </aside>
            ...
            <div class="absolute inset-y-0 right-0 h-full w-full overflow-y-auto bg-[#f8fbff] shadow-2xl 2xl:max-w-[1240px]">
                <div class="sticky top-0 z-20 flex items-center justify-between border-b px-5 py-6 sm:px-8 transition-all duration-500" :class="modalMode === 'edit' ? 'bg-slate-900 border-slate-800' : 'bg-[#EEF2FF] border-[#E0E7FF]'">
                    <div class="flex items-center gap-5">
                        ...
                        <div>
                            <h2 class="text-2xl font-black tracking-tight transition-colors" :class="modalMode === 'edit' ? 'text-white' : 'text-slate-900'">{{ modalMode === 'edit' ? 'Edit Review' : 'Create Review' }}</h2>
                            <p class="mt-0.5 text-[13px] font-medium transition-colors" :class="modalMode === 'edit' ? 'text-slate-400' : 'text-slate-500'">{{ modalMode === 'edit' ? 'Update bad review details.' : 'Input new bad review into the system.' }}</p>
                        </div>
                    </div>

                    <div class="flex items-center gap-3">
                        <button type="button" class="h-11 rounded-xl px-5 text-sm font-bold transition-all active:scale-95" :class="modalMode === 'edit' ? 'text-slate-400 hover:bg-white/5' : 'text-slate-500 hover:bg-slate-100'" @click="discardForm">
                            Cancel
                        </button>
                        <button type="button" class="h-11 rounded-xl px-6 text-sm font-black text-white shadow-xl transition-all hover:-translate-y-0.5 active:scale-95 disabled:cursor-not-allowed disabled:opacity-50" :class="modalMode === 'edit' ? 'bg-blue-600 shadow-blue-500/20 hover:bg-blue-500' : 'bg-[var(--app-primary)] shadow-indigo-500/20 hover:bg-[var(--app-primary-dark)]'" :disabled="form.processing" @click="submitForm">
                            <div class="flex items-center gap-2">
                                ...
                                <span>{{ form.processing ? 'Saving...' : (modalMode === 'edit' ? 'Save Changes' : 'Submit Review') }}</span>
                            </div>
                        </button>
                    </div>
                </div>
                    ...
                    <div class="mx-auto grid max-w-[1160px] gap-8 xl:grid-cols-[minmax(0,1fr)_320px]">
                        <div class="space-y-7">
                            ...
                            <div v-if="Object.keys(form.errors).length" class="rounded-[18px] border border-rose-200 bg-rose-50 px-4 py-4 text-sm text-rose-700">
                                <p class="font-semibold">Please fix these errors:</p>
                            ...
                            <section class="rounded-[24px] border border-slate-100 bg-white p-6 shadow-[0_15px_40px_rgba(15,23,42,0.03)] transition-all hover:shadow-[0_20px_50px_rgba(15,23,42,0.05)]">
                                <div class="mb-6 border-b border-slate-50 pb-5">
                                    ...
                                    <h3 class="mt-2 text-lg font-black text-slate-900">Review Information</h3>
                                    <p class="mt-1 text-[13px] font-medium leading-relaxed text-slate-500">
                                        Capture the review details and customer information.
                                    </p>
                                </div>
                                ...
                                <div class="space-y-4">
                                    <div class="grid gap-5 sm:grid-cols-2">
                                        <div class="space-y-2">
                                            <label class="block text-[13px] font-bold uppercase tracking-wide text-slate-700">Tanggal Review*</label>
                                            <input v-model="form.tanggal_review" type="date" :class="controlClass('tanggal_review')" />
                                            <p v-if="fieldError('tanggal_review')" class="text-xs font-medium text-rose-600">{{ fieldError('tanggal_review') }}</p>
                                        </div>
                                        <div class="space-y-2">
                                            <label class="block text-[13px] font-bold uppercase tracking-wide text-slate-700">Star Rating*</label>
                                            <div class="relative">
                                            ...
                                            <p v-if="fieldError('star')" class="text-xs font-medium text-rose-600">{{ fieldError('star') }}</p>
                                        </div>
                                    </div>

                                    <div class="grid gap-5 sm:grid-cols-2">
                                        <div class="space-y-2">
                                            <label class="block text-[13px] font-bold uppercase tracking-wide text-slate-700">Order ID*</label>
                                    ...
                                    <div class="grid gap-5 sm:grid-cols-2">
                                        <div class="space-y-2">
                                            <label class="block text-[13px] font-bold uppercase tracking-wide text-slate-700">SKU</label>
                                            <div class="relative">
                                                <select v-model="form.sku" :class="controlClass('sku', 'select')" @change="handleSkuChange">
                                                    <option value="">Select SKU</option>
                                                    <option v-for="option in skuCodeOptions" :key="option.sku" :value="option.sku">{{ option.sku }}</option>
                                                </select>
                                                <ChevronDown class="pointer-events-none absolute right-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
                                            </div>
                                        </div>
                            ...
                            <section class="rounded-[24px] border border-slate-100 bg-white p-6 shadow-[0_15px_40px_rgba(15,23,42,0.03)] transition-all hover:shadow-[0_20px_50px_rgba(15,23,42,0.05)]">
                                <div class="mb-6 border-b border-slate-50 pb-5">
                                    ...
                                    <h3 class="mt-2 text-lg font-black text-slate-900">Review Details</h3>
                                    <p class="mt-1 text-[13px] font-medium leading-relaxed text-slate-500">
                                        Capture the issue category, cause, and detailed notes.
                                    </p>
                                </div>
                                ...
                                <div class="space-y-4">
                                    <div class="grid gap-5 sm:grid-cols-2">
                                        <div class="space-y-2">
                                            <label class="block text-[13px] font-bold uppercase tracking-wide text-slate-700">Category Review*</label>
                                            <div class="relative">
                                                <select v-model="form.category_review" :class="controlClass('category_review', 'select')">
                                                    <option value="">Select Category</option>
                                                    <option v-for="option in categoryReviewOptions" :key="option" :value="option">{{ option }}</option>
                                                </select>
                                                <ChevronDown class="pointer-events-none absolute right-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
                                            </div>
                                            <p v-if="fieldError('category_review')" class="text-xs font-medium text-rose-600">{{ fieldError('category_review') }}</p>
                                        </div>
                                        <div class="space-y-2">
                                            <label class="block text-[13px] font-bold uppercase tracking-wide text-slate-700">By (Cause)</label>
                                            <input v-model="form.cause_by" type="text" :class="readonlyInputClass" readonly />
                                        </div>
                                    </div>

                                    <div class="space-y-2">
                                        <label class="block text-[13px] font-bold uppercase tracking-wide text-slate-700">Review Notes*</label>
                                        <textarea v-model="form.review_notes" rows="4" :class="controlClass('review_notes', 'textarea')"></textarea>
                                        <p v-if="fieldError('review_notes')" class="text-xs font-medium text-rose-600">{{ fieldError('review_notes') }}</p>
                                    </div>

                                    <div class="grid gap-5 sm:grid-cols-2">
                                        <div class="space-y-2">
                                            <label class="block text-[13px] font-bold uppercase tracking-wide text-slate-700">Progress*</label>
                                            <div class="relative">
                                            ...
                                            <p v-if="fieldError('progress')" class="text-xs font-medium text-rose-600">{{ fieldError('progress') }}</p>
                                        </div>
                                        <div class="space-y-2">
                                            <label class="block text-[13px] font-bold uppercase tracking-wide text-slate-700">CS/Reviewer Name*</label>
                                            <div class="relative">
                        ...
                        <aside class="space-y-6">
                            <div class="sticky top-12 space-y-6">
                                <div class="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm ring-1 ring-slate-100/50">
                                    <p class="text-[9px] font-black uppercase tracking-[0.2em] text-slate-400">Form Info</p>
                                    <div class="mt-4 space-y-4">
                                        <div class="rounded-xl border border-slate-50 bg-slate-50/50 p-3.5">
                                            <p class="text-[10px] font-bold text-slate-400">Status</p>
                                            <div class="mt-1.5 inline-flex rounded-full px-2.5 py-0.5 text-[9px] font-black uppercase tracking-wider shadow-sm" :class="form.progress === 'Auto Reply' ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'">
                                                {{ form.progress === 'Auto Reply' ? 'SOLVED' : 'PENDING' }}
                                            </div>
                                        </div>

                                        <div class="rounded-xl border border-slate-50 bg-slate-50/50 p-3.5">
                                            <p class="text-[10px] font-bold text-slate-400">Month (Auto)</p>
                                            <p class="mt-0.5 text-[11px] font-bold text-slate-700">{{ form.tanggal_review ? new Date(form.tanggal_review + 'T00:00:00').toLocaleDateString('en-US', { month: 'long', year: 'numeric' }) : '-' }}</p>
                                        </div>
                                    </div>
                                </div>

                                <div class="rounded-2xl border border-slate-100 bg-[var(--app-ink)] p-6 text-white shadow-xl">
                                    <p class="text-[9px] font-black uppercase tracking-[0.2em] text-white/40">Guidelines</p>
                                    <ul class="mt-4 space-y-3">
                                        <li class="flex items-start gap-2.5">
                                            ...
                                            <p class="text-[12px] font-medium leading-tight text-white/80">
                                                Mark reviewed status based on <span class="font-bold text-white">Progress</span>.
                                            </p>
                                        </li>
                                        <li class="flex items-start gap-2.5">
                                            <div class="flex h-4.5 w-4.5 shrink-0 items-center justify-center rounded-full bg-white/10 text-white">
                                            ...
                                            <p class="text-[12px] font-medium leading-tight text-white/80">
                                                Status auto-updates: <span class="font-bold text-white">Follow Up = Pending</span>, <span class="font-bold text-white">Auto Reply = Solved</span>.
                                            </p>
                                        </li>
                                        <li class="flex items-start gap-2.5">
                                            <div class="flex h-4.5 w-4.5 shrink-0 items-center justify-center rounded-full bg-white/10 text-white">
                                            ...
                                            <p class="text-[12px] font-medium leading-tight text-white/80">
                                                Month auto-calculates from review date for reporting.
                                            </p>
                                        </li>
                                    </ul>
                                </div>
    ...
    <transition name="fade">
        <div v-if="isDeleteModalOpen" class="fixed inset-0 z-[60] flex items-center justify-center bg-slate-950/40 px-4 backdrop-blur-sm" @click.self="isDeleteModalOpen = false">
            <div class="w-full max-w-sm rounded-2xl bg-white p-6 shadow-2xl">
                ...
                <h3 class="mt-4 text-lg font-black text-slate-900">Delete Review</h3>
                <p class="mt-2 text-sm font-medium text-slate-500">
                    Are you sure you want to delete this review? This action cannot be undone.
                </p>
                <div class="mt-6 flex gap-3">
                    <button type="button" class="flex-1 rounded-lg border border-slate-200 px-4 py-2.5 text-sm font-bold text-slate-600 transition hover:bg-slate-50" @click="isDeleteModalOpen = false">
                        Cancel
                    </button>
                    <button type="button" class="flex-1 rounded-lg bg-rose-600 px-4 py-2.5 text-sm font-bold text-white transition hover:bg-rose-700 disabled:opacity-50" :disabled="form.processing" @click="deleteRecord">
                        Delete
                    </button>
                </div>
            </div>
