        ...
        <div class="space-y-4">
            <section class="app-table-shell overflow-hidden">
                <div class="flex flex-col gap-4 border-b border-[var(--app-border)] px-5 py-4 lg:flex-row lg:items-center lg:justify-between">
                    <div>
                        <p class="text-[10px] font-black uppercase tracking-[0.14em] text-slate-400">Stock Incident</p>
                        <h2 class="mt-1 text-xl font-black tracking-tight text-[var(--app-ink)]">OOS Workspace</h2>
                        <p class="mt-1 text-xs font-medium text-slate-500">Monitoring stock issue secara real-time demi kelancaran operasional.</p>
                    </div>
                ...
                <div class="grid gap-3.5 px-4 py-4 md:grid-cols-3">
                    <article v-for="card in metrics" :key="card.label" class="rounded-xl border border-slate-50 bg-white px-4 py-4 shadow-sm transition-all hover:border-[var(--app-primary)]/20">
            ...
            <section class="app-table-shell overflow-hidden">
                <div class="border-b border-[var(--app-border)] px-5 py-4">
                    <h3 class="text-base font-black text-[var(--app-ink)] uppercase tracking-wide">OOS List</h3>
                </div>
                    ...
                    <table class="min-w-full divide-y divide-[var(--app-border)]">
                        <thead class="bg-slate-50/50">
                            <tr class="text-left text-[10px] font-black uppercase tracking-widest text-slate-400">
                                ...
                                <th class="px-5 py-3">Tanggal Input</th>
                        ...
                        <tbody class="divide-y divide-[var(--app-border)] bg-white text-[13px]">
                            <tr v-for="item in rows" :key="item.id" class="hover:bg-slate-50/50 transition-colors">
                                ...
                                <td class="px-5 py-3 font-medium text-slate-600">{{ item.tanggal_input || '-' }}</td>
                                <td class="px-5 py-3 font-medium text-slate-600">{{ item.brand || '-' }}</td>
                                <td class="px-5 py-3 font-medium text-slate-600">{{ item.platform || '-' }}</td>
                                <td class="px-5 py-3 font-medium text-slate-600">{{ item.sku || '-' }}</td>
                                <td class="px-5 py-3 font-medium text-slate-600">{{ item.month || '-' }}</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                ...
                <div v-if="!rows.length" class="px-6 py-10 text-center">
                    <p class="text-base font-bold text-[var(--app-ink)]">Belum ada data OOS</p>
                    <p class="mt-1 text-xs text-slate-500">Incident stok akan muncul di sini.</p>
                </div>
            </section>
        </div>
    </AppLayout>
