...
const props = defineProps<{
    platforms: Paginator<ManagedPlatform>;
    filters: { search?: string | null; status?: string | null; };
    metrics: { total: number; active: number; inactive: number; };
}>();

const breadcrumbs: BreadcrumbItem[] = [{ title: 'Dashboard', href: '/dashboard' }, { title: 'Master Platforms', href: '/platforms' }];
...
const activePlatform = ref<ManagedPlatform | null>(null);

const canCreatePlatforms = computed(() => page.props.auth.can.create_platforms);
...
const platformRows = computed(() => props.platforms.data ?? []);
const paginationLinks = computed(() => props.platforms.links?.filter((link) => link.url) ?? []);
const summaryCards = computed(() => [
    { label: 'Total Platform', value: props.metrics.total, icon: Store, tone: 'bg-[var(--app-primary)] text-white shadow-[0_12px_24px_rgba(53,103,232,0.22)]' },
...
const visitIndex = (overrides: Record<string, unknown> = {}, replace = true) => {
    router.get(route('platforms.index'), {
...
const openDeleteModal = (platform: ManagedPlatform) => {
    activePlatform.value = platform;
...
const submitCreate = () => createForm.post(route('platforms.store'), { preserveScroll: true, onSuccess: () => { isCreateOpen.value = false; resetCreateForm(); } });
const submitEdit = () => {
    if (!activePlatform.value) return;
    editForm.put(route('platforms.update', activePlatform.value.id), { preserveScroll: true, onSuccess: () => { isEditOpen.value = false; activePlatform.value = null; editForm.clearErrors(); } });
};
const submitDelete = () => {
    if (!activePlatform.value) return;
    deleteForm.delete(route('platforms.destroy', activePlatform.value.id), { preserveScroll: true, onSuccess: () => closeDeleteModal() });
};
                ...
                <section class="app-table-shell overflow-hidden">
                    <div class="grid gap-6 border-b border-[var(--app-border)] bg-[linear-gradient(135deg,_#eef4ff_0%,_#f8fbff_100%)] px-6 py-7 text-[var(--app-ink)] lg:grid-cols-[1.2fr,0.8fr]">
                        <div>
                            ...
                            <h1 class="mt-3 text-3xl font-extrabold tracking-tight">Platform Directory</h1>
                            <p class="mt-3 max-w-2xl text-sm leading-6 text-slate-500">
                                Kelola daftar platform marketplace agar semua modul menggunakan source option yang sama dan lebih konsisten.
                            </p>
                        </div>

                        <div v-if="canCreatePlatforms" class="flex items-start justify-end">
                ...
                <section class="app-table-shell p-6">
                    <div class="grid gap-5 xl:grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)] xl:items-end">
                        <div class="min-w-0 max-w-xl">
                            <h2 class="text-xl font-semibold text-slate-900">Platform List</h2>
                            <p class="mt-1 max-w-md text-sm leading-6 text-slate-500">
                                Master platform disiapkan lebih awal supaya form Complaint, Bad Reviews, Tracking, dan OOS tidak lagi bergantung pada option hardcoded.
                            </p>
                        </div>
                        <div class="grid gap-3 sm:grid-cols-2 xl:grid-cols-[minmax(0,1.5fr)_minmax(0,0.8fr)]">
                            <div class="relative">
                                <Search class="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
                                <Input v-model="search" class="pl-10" placeholder="Cari nama platform..." />
                            </div>
                            ...
                            <table class="min-w-full divide-y divide-slate-200 text-sm">
                                <thead class="bg-slate-50/90">
                                    <tr class="text-left text-slate-500">
                                        <th class="px-5 py-4 font-medium">Platform</th>
                                ...
                                <tbody class="divide-y divide-slate-100 bg-white">
                                    <tr v-for="platform in platformRows" :key="platform.id" class="transition hover:bg-slate-50/70">
                                        <td class="px-5 py-4">
                                            <div class="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <p class="font-medium text-slate-900">{{ platform.name }}</p>
                                                    <p class="mt-1 text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">Master Platform</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="px-5 py-4">
                                            <span class="inline-flex rounded-full px-3 py-1 text-xs font-semibold" :class="statusBadgeClass(platform.is_active)">
                                                {{ platform.is_active ? 'Active' : 'Inactive' }}
                                            </span>
                                        </td>
                                        <td class="px-5 py-4 text-slate-500">{{ formatDate(platform.created_at) }}</td>
                                        <td class="px-5 py-4">
                                            <div class="flex justify-end gap-2">
                                                <Button v-if="canUpdatePlatforms" type="button" variant="outline" size="sm" class="rounded-xl" @click="openEditModal(platform)">
                                                    <PencilLine class="h-4 w-4" />
                                                    Edit
                                                </Button>
                                                <Button v-if="canDeletePlatforms" type="button" variant="destructive" size="sm" class="rounded-xl" @click="openDeleteModal(platform)">
                                    ...
                                    <tr v-if="platformRows.length === 0">
                                        <td colspan="4" class="px-5 py-14 text-center">
                                            <div class="mx-auto max-w-sm space-y-2">
                                                ...
                                                <p class="font-medium text-slate-900">Tidak ada platform yang cocok</p>
                    ...
                    <div class="mt-5 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                        <p class="text-sm text-slate-500">Menampilkan {{ platformsPage.from || 0 }} - {{ platformsPage.to || 0 }} dari {{ platformsPage.total }} platform</p>
            ...
            <DialogContent class="max-w-xl rounded-[28px] border-0 p-0 shadow-[0_30px_80px_rgba(15,23,42,0.25)]">
                <div class="overflow-hidden rounded-[28px]">
                    <div class="bg-[linear-gradient(135deg,_#eef4ff_0%,_#dbeafe_100%)] px-6 py-6 text-[var(--app-ink)]">
                        <DialogHeader>
                            <DialogTitle class="text-2xl">Tambah Platform</DialogTitle>
                            <DialogDescription class="text-slate-500">Masukkan platform baru agar siap dipakai sebagai master data.</DialogDescription>
                        </DialogHeader>
                    </div>
                    ...
                    <form class="space-y-5 bg-white px-6 py-6" @submit.prevent="submitCreate">
                        <div class="grid gap-5">
                            <div class="grid gap-2">
                                <Label for="create-name">Nama Platform</Label>
                                <Input id="create-name" v-model="createForm.name" placeholder="Contoh: SHOPEE" />
                            ...
                            <div class="flex items-end rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
                                <label class="flex items-center gap-3 text-sm font-medium text-slate-700">
                                    ...
                                    Platform aktif dan siap dipakai di modul lain
                                </label>
                            </div>
                        </div>
                        ...
                        <DialogFooter class="border-t border-slate-100 pt-5">
                            ...
                            <Button :disabled="createForm.processing" class="rounded-xl">
                                <Plus class="h-4 w-4" />
                                {{ createForm.processing ? 'Creating...' : 'Create Platform' }}
                            </Button>
                        </DialogFooter>
                    </form>
                </div>
        ...
        <Dialog v-model:open="isEditOpen">
            <DialogContent class="max-w-xl rounded-[28px] border-0 p-0 shadow-[0_30px_80px_rgba(15,23,42,0.25)]">
                <div class="overflow-hidden rounded-[28px]">
                    <div class="bg-[linear-gradient(135deg,_#eef4ff_0%,_#dbeafe_100%)] px-6 py-6 text-[var(--app-ink)]">
                        <DialogHeader>
                            <DialogTitle class="text-2xl">Edit Platform</DialogTitle>
                            <DialogDescription class="text-slate-500">Perbarui nama atau status aktif platform agar master data tetap rapi.</DialogDescription>
                        </DialogHeader>
                    </div>

                    <form v-if="activePlatform" class="space-y-5 bg-white px-6 py-6" @submit.prevent="submitEdit">
                        <div class="grid gap-5">
                            <div class="grid gap-2">
                                <Label for="edit-name">Nama Platform</Label>
            ...
            <DialogContent class="max-w-md rounded-[28px] border-0 p-0 shadow-[0_30px_80px_rgba(15,23,42,0.25)]">
                <div class="overflow-hidden rounded-[28px] bg-white">
                    <div class="bg-[linear-gradient(135deg,_#fff1f2_0%,_#ffe4e6_100%)] px-6 py-6 text-[var(--app-ink)]">
                        <DialogHeader>
                            <DialogTitle class="text-2xl">Hapus Platform</DialogTitle>
                            <DialogDescription class="text-rose-500">Tindakan ini tidak bisa dibatalkan. Pastikan platform memang aman untuk dihapus dari master data.</DialogDescription>
                        </DialogHeader>
                    </div>

                    <div class="space-y-5 px-6 py-6">
                        <div v-if="activePlatform" class="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                            <p class="font-medium text-slate-900">{{ activePlatform.name }}</p>
                            <div class="mt-3 flex flex-wrap gap-2">
                                <span class="inline-flex rounded-full px-3 py-1 text-xs font-semibold" :class="statusBadgeClass(activePlatform.is_active)">
                                    {{ activePlatform.is_active ? 'Active' : 'Inactive' }}
                                </span>
                            </div>
                        </div>
                        ...
                        <DialogFooter>
                            ...
                            <Button type="button" variant="destructive" class="rounded-xl" :disabled="deleteForm.processing" @click="submitDelete">
                                <Trash2 class="h-4 w-4" />
                                {{ deleteForm.processing ? 'Deleting...' : 'Delete Platform' }}
                            </Button>
                        </DialogFooter>
                    </div>
                </div>
