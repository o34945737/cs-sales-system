            ...
            <div class="mx-auto max-w-7xl space-y-4">
                <section class="app-table-shell overflow-hidden">
                    <div class="grid gap-4 border-b border-[var(--app-border)] bg-[linear-gradient(135deg,_#eef4ff_0%,_#f8fbff_100%)] px-5 py-4 text-[var(--app-ink)] lg:grid-cols-[1.2fr,0.8fr]">
                        <div>
                            ...
                            <h1 class="mt-1 text-xl font-black tracking-tight">Platform Directory</h1>
                            <p class="mt-1 text-xs font-medium text-slate-500">
                                Kelola daftar platform marketplace agar semua modul menggunakan source yang konsisten.
                            </p>
                        </div>
 
                        <div v-if="canCreatePlatforms" class="flex items-center justify-end">
                ...
                <section class="app-table-shell p-5">
                    <div class="grid gap-4 xl:grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)] xl:items-center">
                        <div>
                            <h2 class="text-base font-black text-slate-900 uppercase tracking-wide">Platform List</h2>
                        </div>
                        <div class="grid gap-2 sm:grid-cols-2 lg:flex lg:justify-end">
                            <div class="relative lg:w-64">
                                <Search class="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
                                <Input v-model="search" class="h-9 pl-9 text-xs" placeholder="Cari platform..." />
                            </div>
                        ...
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-slate-100 text-[13px]">
                                ...
                                <tbody class="divide-y divide-slate-50 bg-white">
                                    <tr v-for="platform in platformRows" :key="platform.id" class="transition hover:bg-slate-50/50">
                                        <td class="px-4 py-3">
                                            <div class="flex items-center gap-2.5">
                                                ...
                                                <div>
                                                    <p class="font-bold text-slate-900 leading-tight">{{ platform.name }}</p>
                                                    <p class="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">ID: #{{ platform.id }}</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="px-4 py-3">
                                            <span class="inline-flex rounded-full px-2 py-0.5 text-[9px] font-black uppercase tracking-wider shadow-sm" :class="statusBadgeClass(platform.is_active)">
                                        ...
                                        <td class="px-4 py-3 text-right">
                                            <div class="flex justify-end gap-1.5">
                                                ...
                                                <Button v-if="canDeletePlatforms" type="button" variant="ghost" size="sm" class="h-8 rounded-lg text-slate-400 hover:text-rose-600 hover:bg-rose-50" @click="openDeleteModal(platform)">
                    ...
                    <div class="mt-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                        <p class="text-[11px] font-bold text-slate-400">Showing {{ platformsPage.from || 0 }}-{{ platformsPage.to || 0 }} of {{ platformsPage.total }} entries</p>
        ...
        <Dialog v-model:open="isCreateOpen">
            <DialogContent class="max-w-xl overflow-hidden rounded-[28px] border-0 p-0 shadow-[0_30px_80px_rgba(15,23,42,0.25)]">
                <div class="bg-[#EEF2FF] px-7 py-8">
                    <DialogHeader>
                        <DialogTitle class="text-3xl font-black text-slate-900">Tambah Platform</DialogTitle>
                        <DialogDescription class="mt-2 text-base font-medium text-slate-500">Masukkan platform baru agar siap dipakai sebagai master data.</DialogDescription>
                    </DialogHeader>
                </div>
                ...
                <form class="space-y-6 bg-white px-7 py-7" @submit.prevent="submitCreate">
                    <div class="space-y-4">
                        <div class="grid gap-2">
                            <Label for="create-name" class="text-[13px] font-bold uppercase tracking-wide text-slate-700">Nama Platform</Label>
        ...
        <Dialog v-model:open="isEditOpen">
            <DialogContent class="max-w-xl overflow-hidden rounded-[28px] border-0 p-0 shadow-[0_30px_80px_rgba(15,23,42,0.25)]">
                <div class="bg-slate-900 px-7 py-8">
                    <DialogHeader>
                        <DialogTitle class="text-3xl font-black text-white">Edit Platform</DialogTitle>
                        <DialogDescription class="mt-2 text-base font-medium text-slate-400">Perbarui nama atau status aktif platform agar operasional tetap konsisten.</DialogDescription>
                    </DialogHeader>
                </div>

                <form v-if="activePlatform" class="space-y-6 bg-white px-7 py-7" @submit.prevent="submitEdit">
                    <div class="space-y-4">
                        <div class="grid gap-2">
                        ...
                        <div class="flex items-center gap-3 rounded-2xl border border-slate-100 bg-slate-50/50 px-4 py-4 transition hover:bg-slate-50">
                            ...
                            <span class="text-sm font-semibold text-slate-600">Platform aktif</span>
                        </div>
                    </div>
        ...
        <Dialog v-model:open="isDeleteOpen">
            <DialogContent class="max-w-md overflow-hidden rounded-[28px] border-0 p-0 shadow-[0_30px_80px_rgba(244,63,94,0.15)]">
                <div class="bg-rose-50 px-7 py-8">
                    <DialogHeader>
                        <DialogTitle class="text-3xl font-black text-rose-950">Hapus Platform</DialogTitle>
                ...
                <div class="space-y-6 bg-white px-7 py-7">
                    <div v-if="activePlatform" class="rounded-2xl border border-rose-100 bg-rose-50/30 p-5">
                        <p class="text-sm font-bold text-rose-900 uppercase tracking-tight">Konfirmasi Penghapusan</p>
                        <p class="mt-1 text-lg font-black text-rose-600">{{ activePlatform.name }}</p>
                    </div>
