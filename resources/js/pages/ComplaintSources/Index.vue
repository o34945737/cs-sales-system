                ...
                <section class="app-table-shell overflow-hidden">
                    ...
                    >
                        <div>
                            ...
                            <h1 class="mt-3 text-3xl font-extrabold tracking-tight">Complaint Source Directory</h1>
                            <p class="mt-3 max-w-2xl text-sm leading-6 text-slate-500">
                                Kelola daftar source complaint agar form complaint mengambil source dari master yang sama, bukan hardcoded.
                            </p>
                        </div>

                        <div v-if="canCreateComplaintSources" class="flex items-start justify-end">
                            <Button
                                type="button"
                                size="lg"
                ...
                <section class="app-table-shell p-6">
                    <div class="grid gap-5 xl:grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)] xl:items-end">
                        <div class="min-w-0 max-w-xl">
                            <h2 class="text-xl font-semibold text-slate-900">Complaint Source List</h2>
                            <p class="mt-1 max-w-md text-sm leading-6 text-slate-500">
                                Source complaint dipisahkan ke master agar perubahan opsi di tim operasional tidak perlu edit kode lagi.
                            </p>
                        </div>
                        <div class="grid gap-3 sm:grid-cols-2 xl:grid-cols-[minmax(0,1.5fr)_minmax(0,0.8fr)]">
                            <div class="relative">
                                <Search class="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" />
                                <Input v-model="search" class="pl-10" placeholder="Cari source complaint..." />
                            </div>
                            ...
                            <table class="min-w-full divide-y divide-slate-200 text-sm">
                                <thead class="bg-slate-50/90">
                                    <tr class="text-left text-slate-500">
                                        <th class="px-5 py-4 font-medium">Source</th>
                                ...
                                <tbody class="divide-y divide-slate-100 bg-white">
                                    <tr v-for="complaintSource in rows" :key="complaintSource.id" class="transition hover:bg-slate-50/70">
                                        <td class="px-5 py-4">
                                            <div class="flex items-center gap-3">
                                                ...
                                                <div>
                                                    <p class="font-medium text-slate-900">{{ complaintSource.name }}</p>
                                                    <p class="mt-1 text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">
                                                        Master Complaint Source
                                                    </p>
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td class="px-5 py-4 text-slate-500">{{ formatDate(complaintSource.created_at) }}</td>
                                        <td class="px-5 py-4">
                                    ...
                                    <tr v-if="rows.length === 0">
                                        <td colspan="4" class="px-5 py-14 text-center">
                                            <div class="mx-auto max-w-sm space-y-2">
                                                ...
                                                <p class="font-medium text-slate-900">Tidak ada complaint source yang cocok</p>
                    ...
                    <div class="mt-5 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                        <p class="text-sm text-slate-500">
                            Menampilkan {{ pageData.from || 0 }} - {{ pageData.to || 0 }} dari {{ pageData.total }} source
                        </p>
            ...
            <DialogContent class="max-w-xl rounded-[28px] border-0 p-0 shadow-[0_30px_80px_rgba(15,23,42,0.25)]">
                <div class="overflow-hidden rounded-[28px]">
                    <div class="bg-[linear-gradient(135deg,_#eef4ff_0%,_#dbeafe_100%)] px-6 py-6 text-[var(--app-ink)]">
                        <DialogHeader>
                            <DialogTitle class="text-2xl">Tambah Complaint Source</DialogTitle>
                            <DialogDescription class="text-slate-500"
                                >Masukkan source complaint baru agar siap dipakai di flow complaint.</DialogDescription
                            >
                        </DialogHeader>
                    </div>
                    ...
                    <form class="space-y-5 bg-white px-6 py-6" @submit.prevent="submitCreate">
                        <div class="grid gap-2">
                            <Label for="create-name">Nama Source</Label>
                            <Input id="create-name" v-model="createForm.name" placeholder="Contoh: After Sales" />
                        ...
                        <label class="flex items-center gap-3 text-sm text-slate-600">
                            ...
                            />
                            Source aktif dan siap dipakai di complaint
                        </label>
            ...
            <DialogContent class="max-w-xl rounded-[28px] border-0 p-0 shadow-[0_30px_80px_rgba(15,23,42,0.25)]">
                <div class="overflow-hidden rounded-[28px]">
                    <div class="bg-[linear-gradient(135deg,_#0f172a_0%,_#1e293b_100%)] px-6 py-6 text-white">
                        <DialogHeader>
                            <DialogTitle class="text-2xl">Edit Complaint Source</DialogTitle>
                            <DialogDescription class="text-slate-200">Perbarui source complaint yang dipakai flow complaint.</DialogDescription>
                        </DialogHeader>
                    </div>

                    <form v-if="activeComplaintSource" class="space-y-5 bg-white px-6 py-6" @submit.prevent="submitEdit">
                        <div class="grid gap-2">
                            <Label for="edit-name">Nama Source</Label>
            ...
            <DialogContent class="max-w-lg rounded-[28px] border-0 p-0 shadow-[0_30px_80px_rgba(15,23,42,0.25)]">
                <div class="overflow-hidden rounded-[28px]">
                    <div class="bg-rose-50 px-6 py-6 text-rose-700">
                        <DialogHeader>
                            <DialogTitle class="text-2xl text-rose-700">Hapus Complaint Source</DialogTitle>
                            <DialogDescription class="text-rose-600"
                                >Pastikan source ini memang tidak lagi dipakai di flow complaint.</DialogDescription
                            >
                        </DialogHeader>
                    </div>

                    <div v-if="activeComplaintSource" class="space-y-5 bg-white px-6 py-6">
                        <div class="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                            <p class="font-medium text-slate-900">{{ activeComplaintSource.name }}</p>
                            <span
                                class="mt-2 inline-flex rounded-full px-3 py-1 text-xs font-semibold"
                                :class="statusBadgeClass(activeComplaintSource.is_active)"
                            >
                                {{ activeComplaintSource.is_active ? 'Active' : 'Inactive' }}
                            </span>
                        </div>
