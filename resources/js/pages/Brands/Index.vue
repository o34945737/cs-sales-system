...
import { CheckCircle2, ChevronLeft, ChevronRight, PencilLine, Plus, Search, Tag, Tags, Trash2, XCircle } from 'lucide-vue-next';
import { computed, ref, watch } from 'vue';

interface ManagedBrand { id: number; name: string; is_active: boolean; created_at: string | null; }
...
const props = defineProps<{
    brands: Paginator<ManagedBrand>;
    filters: { search?: string | null; status?: string | null; };
    metrics: { total: number; active: number; inactive: number; };
}>();

const breadcrumbs: BreadcrumbItem[] = [{ title: 'Dashboard', href: '/dashboard' }, { title: 'Master Brands', href: '/brands' }];
...
const activeBrand = ref<ManagedBrand | null>(null);

const canCreateBrands = computed(() => page.props.auth.can.create_brands);
const canUpdateBrands = computed(() => page.props.auth.can.update_brands);
const canDeleteBrands = computed(() => page.props.auth.can.delete_brands);

const createForm = useForm({ name: '', is_active: true });
const editForm = useForm({ name: '', is_active: true });
const deleteForm = useForm({});

const brandsPage = computed(() => props.brands);
const brandRows = computed(() => props.brands.data ?? []);
const paginationLinks = computed(() => props.brands.links?.filter((link) => link.url) ?? []);
const summaryCards = computed(() => [
    { label: 'Total Brand', value: props.metrics.total, icon: Tags, tone: 'bg-[var(--app-primary)] text-white shadow-[0_12px_24px_rgba(53,103,232,0.22)]' },
...
const openEditModal = (brand: ManagedBrand) => {
    activeBrand.value = brand;
    editForm.defaults({ name: brand.name, is_active: brand.is_active });
    editForm.reset();
    editForm.clearErrors();
    isEditOpen.value = true;
};

const openDeleteModal = (brand: ManagedBrand) => {
    activeBrand.value = brand;
...
const submitCreate = () => createForm.post(route('brands.store'), { preserveScroll: true, onSuccess: () => { isCreateOpen.value = false; resetCreateForm(); } });
const submitEdit = () => {
    if (!activeBrand.value) return;
    editForm.put(route('brands.update', activeBrand.value.id), { preserveScroll: true, onSuccess: () => { isEditOpen.value = false; activeBrand.value = null; editForm.clearErrors(); } });
};
const submitDelete = () => {
    if (!activeBrand.value) return;
    deleteForm.delete(route('brands.destroy', activeBrand.value.id), { preserveScroll: true, onSuccess: () => closeDeleteModal() });
};
            ...
            <div class="mx-auto max-w-7xl space-y-4">
                <section class="app-table-shell overflow-hidden">
                    <div class="grid gap-4 border-b border-[var(--app-border)] bg-[linear-gradient(135deg,_#eef4ff_0%,_#f8fbff_100%)] px-5 py-4 text-[var(--app-ink)] lg:grid-cols-[1.2fr,0.8fr]">
                        <div>
                            ...
                            <h1 class="mt-1 text-xl font-black tracking-tight">Brand Directory</h1>
                            <p class="mt-1 text-xs font-medium text-slate-500">
                                Kelola daftar brand aktif sebagai fondasi integrasi data operasional.
                            </p>
                        </div>
 
                        <div v-if="canCreateBrands" class="flex items-center justify-end">
                    ...
                    <div class="grid gap-3 px-4 py-3 md:grid-cols-3">
                ...
                <section class="app-table-shell p-5">
                    <div class="grid gap-4 xl:grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)] xl:items-center">
                        <div>
                            <h2 class="text-base font-black text-slate-900 uppercase tracking-wide">Brand List</h2>
                        </div>
                        <div class="grid gap-2 sm:grid-cols-2 lg:flex lg:justify-end">
                            <div class="relative lg:w-64">
                                ...
                                <Input v-model="search" class="h-9 pl-9 text-xs" placeholder="Cari nama brand..." />
                            </div>
                        ...
                        <div class="overflow-x-auto">
                            <table class="min-w-full divide-y divide-slate-100 text-[13px]">
                                ...
                                <tbody class="divide-y divide-slate-50 bg-white">
                                    <tr v-for="brand in brandRows" :key="brand.id" class="transition hover:bg-slate-50/50">
                                        <td class="px-4 py-3">
                                            <div class="flex items-center gap-2.5">
                                                ...
                                                <div>
                                                    <p class="font-bold text-slate-900 leading-tight">{{ brand.name }}</p>
                                                    <p class="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">ID: #{{ brand.id }}</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td class="px-4 py-3">
                                            <span class="inline-flex rounded-full px-2 py-0.5 text-[9px] font-black uppercase tracking-wider shadow-sm" :class="statusBadgeClass(brand.is_active)">
                                                {{ brand.is_active ? 'Active' : 'Inactive' }}
                                            </span>
                                        </td>
                                        <td class="px-4 py-3">
                                            <div class="flex justify-end gap-1.5">
                                                <Button v-if="canUpdateBrands" type="button" variant="ghost" size="sm" class="h-8 rounded-lg text-slate-400 hover:text-[var(--app-primary)] hover:bg-blue-50" @click="openEditModal(brand)">
                                                    <PencilLine class="h-3.5 w-3.5" />
                                                </Button>
                                                <Button v-if="canDeleteBrands" type="button" variant="ghost" size="sm" class="h-8 rounded-lg text-slate-400 hover:text-rose-600 hover:bg-rose-50" @click="openDeleteModal(brand)">
                                                    <Trash2 class="h-3.5 w-3.5" />
                                                </Button>
                                            </div>
                                        </td>
                                    </tr>
                                    <tr v-if="brandRows.length === 0">
                                        <td colspan="3" class="px-4 py-10 text-center">
                                            <p class="text-[13px] font-bold text-slate-400">Tidak ada data ditemukan</p>
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
                    ...
                    <div class="mt-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                        <p class="text-[11px] font-bold text-slate-400">Showing {{ brandsPage.from || 0 }}-{{ brandsPage.to || 0 }} of {{ brandsPage.total }} entries</p>
        ...
        <Dialog v-model:open="isCreateOpen">
            <DialogContent class="max-w-xl overflow-hidden rounded-[28px] border-0 p-0 shadow-[0_30px_80px_rgba(15,23,42,0.25)]">
                <div class="bg-[#EEF2FF] px-7 py-8">
                    <DialogHeader>
                        <DialogTitle class="text-3xl font-black text-slate-900">Tambah Brand</DialogTitle>
                        <DialogDescription class="mt-2 text-base font-medium text-slate-500">Masukkan brand baru agar siap dipakai sebagai master data.</DialogDescription>
                    </DialogHeader>
                </div>
                ...
                <form class="space-y-6 bg-white px-7 py-7" @submit.prevent="submitCreate">
                    <div class="space-y-4">
                        <div class="grid gap-2">
                            <Label for="create-name" class="text-[13px] font-bold uppercase tracking-wide text-slate-700">Nama Brand</Label>
                        ...
                        <div class="flex items-center gap-3 rounded-2xl border border-slate-100 bg-slate-50/50 px-4 py-4 transition hover:bg-slate-50">
                            ...
                            <span class="text-sm font-semibold text-slate-600">Brand aktif dan siap dipakai di modul lain</span>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center justify-end gap-3 border-t border-slate-100 pt-6">
                        ...
                        <Button :disabled="createForm.processing" class="h-11 rounded-xl bg-[var(--app-primary)] px-6 font-bold text-white shadow-lg shadow-blue-500/20 hover:bg-[var(--app-primary-dark)]">
                            ...
                            {{ createForm.processing ? 'Creating...' : 'Create Brand' }}
                        </Button>
                    </div>
                </form>
            </DialogContent>
        ...
        <Dialog v-model:open="isEditOpen">
            <DialogContent class="max-w-xl overflow-hidden rounded-[28px] border-0 p-0 shadow-[0_30px_80px_rgba(15,23,42,0.25)]">
                <div class="bg-slate-900 px-7 py-8">
                    <DialogHeader>
                        <DialogTitle class="text-3xl font-black text-white">Edit Brand</DialogTitle>
                        <DialogDescription class="mt-2 text-base font-medium text-slate-400">Perbarui nama atau status aktif brand agar operasional tetap konsisten.</DialogDescription>
                    </DialogHeader>
                </div>

                <form v-if="activeBrand" class="space-y-6 bg-white px-7 py-7" @submit.prevent="submitEdit">
                    <div class="space-y-4">
                        <div class="grid gap-2">
                            <Label for="edit-name" class="text-[13px] font-bold uppercase tracking-wide text-slate-700">Nama Brand</Label>
                        ...
                        <div class="flex items-center gap-3 rounded-2xl border border-slate-100 bg-slate-50/50 px-4 py-4 transition hover:bg-slate-50">
                            ...
                            <span class="text-sm font-semibold text-slate-600">Brand aktif</span>
                        </div>
                    </div>
        ...
        <Dialog v-model:open="isDeleteOpen">
            <DialogContent class="max-w-md overflow-hidden rounded-[28px] border-0 p-0 shadow-[0_30px_80px_rgba(244,63,94,0.15)]">
                <div class="bg-rose-50 px-7 py-8">
                    <DialogHeader>
                        <DialogTitle class="text-3xl font-black text-rose-950">Hapus Brand</DialogTitle>
                ...
                <div class="space-y-6 bg-white px-7 py-7">
                    <div v-if="activeBrand" class="rounded-2xl border border-rose-100 bg-rose-50/30 p-5">
                        <p class="text-sm font-bold text-rose-900 uppercase tracking-tight">Konfirmasi Penghapusan</p>
                        <p class="mt-1 text-lg font-black text-rose-600">{{ activeBrand.name }}</p>
                    </div>
                    ...
                    <div class="flex items-center justify-end gap-3 border-t border-slate-100 pt-6">
                        ...
                        <Button type="button" class="h-11 rounded-xl bg-rose-600 px-6 font-bold text-white shadow-lg shadow-rose-500/20 hover:bg-rose-700" :disabled="deleteForm.processing" @click="submitDelete">
                            ...
                            {{ deleteForm.processing ? 'Deleting...' : 'Delete Brand' }}
                        </Button>
                    </div>
                </div>
            </DialogContent>
